...
# DORINE - ajout d'un lien vers l'image du fond 
app.py = html.Img(src=app.get_asset_url('trame-background.png')),

# DORINE - AJOUTER UN HEADER /// ÇA MARCHE PAS.
app.layout = html.Div([
    html.Div(
        className="header",
        children=[
            html.Div('Plotly Dash', className="onglet1")
        ]
    ),
    html.Div(
        children=html.Div([
...
# DORINE AJOUTER UN HEADER 

app.layout = html.Div([html.Link(href="https://fonts.googleapis.com/css?family=Lora:400,400i,700,700i|PT+Serif",rel="stylesheet"),
    html.Link(href="https://fonts.googleapis.com/css?family=Lato",rel="stylesheet"),
    html.Link(href="https://fonts.googleapis.com/css?family=Lora:400,400i,700,700i|PT+Serif",rel="stylesheet"),
#DÉBUT DU BLOC ARTICLE   
    html.Div([html.H2("Article 197"),
        html.H3("code général des impôts"),#DORINE - AJOUT DU TITRE
        ...
        html.P(html.Div(["""– """, dcc.Input(id='input-taux0', type='text', value=14,size=sizeperc),"""% pour la fraction supérieure à """,
                         ...
                         """ et inférieure ou égale à""", dcc.Input(id='input-seuil1', type='text', value=27519,size=sizev, className="typovaleur"),"""€ ;"""])),
...
],className="inputarticle",style={'width': '53%', 'display': 'inline-block'}),
#FIN DU BLOC ARTICLE

    html.Div([html.Button(id='submit-button', n_clicks=0, children='Impact sur la population', className="populationbouton"), #DÉPLACEMENT BOUTON COTE STATS

        html.P(dcc.Graph(
        id='graphtotal',className="outputstat"
    )),
        html.P(dcc.Graph(
        id='graphdecile',className="outputstat"
    ))
